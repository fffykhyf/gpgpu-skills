...
| G2 | normalized per-wavefront trace | 调试 multi-wavefront RTL effects |
...
External functional traces 和 normalized per-wavefront RTL traces 适合作为早期 correctness loop，但它们不是 cycle model。
...
|---|---|
| warp scheduler、PC、masks、barriers | scheduler 或 warp state model |
...
- 保持 per-wavefront streams 或稳定 identity fields，避免 interleaving 掩盖 first divergence。
