...
for `gpgpu-contract`. It carries forward the still-active
...
`gpgpu-contract/packs/interconnect`, `gpgpu-contract/packs/memory_path`, and `gpgpu-contract/packs/atomic_sync` produce
`contract_fragment_ir`. `gpgpu-contract` is the only skill that freezes those
