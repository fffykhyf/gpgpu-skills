...
当 GPGPU 变更需要 functional correctness 之外的证据时使用本 skill。PPA 结论必须把 workload、backend、configuration、counters、activity 和 reports 绑定到受控对比中。使用 Rocket Chip 作为把 named configs、local perf events、cache/memory counters、traces、regression flows 和 generated hardware evidence 串起来的参考。
