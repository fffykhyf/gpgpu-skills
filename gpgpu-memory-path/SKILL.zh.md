...
| lane control | active 或 EXEC mask、skipped lanes、per-lane address vector |
...
- response demux 回到 warp、lane mask 和 destination 的路径。
...
- 在 request tag 中丢掉原始 warp/lane/destination metadata。
