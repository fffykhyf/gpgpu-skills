...
- 核心结构：单 SM、warp scheduler、decode/execute pipeline、register file、scoreboard、LSQ、shared memory、global interface。
...
- 实现复杂度理由：避免多 SM 和复杂 cache coherence。
- 性能/面积理由：先保留 LSQ backpressure 可观测性，不追求高吞吐。

## 被拒绝的方案
| 方案 | 拒绝原因 | 后续是否可恢复 |
|---|---|---|
| `MINIMAL_SIMT_CORE` | 缺少 toolchain/runtime/loader vertical slice | 可恢复 |
